        ...
        if (interaction.isChatInputCommand()) {
            ...
            if (result.xp + Math.round((Math.random() * (10 - 1) + 1 / 100 * result.xpBoost) + Math.random() * (10 - 1) + 1) >= result.requiredXp) {
                ...
                if (result.dmNotifs === true) {
                    interaction.user.send({
                        embeds: [new EmbedBuilder().setColor(16777215).setTitle('Congrats, You leveled up!').setDescription(`You are now **level ${result.level}**. As a reward you have been given \`${reward}\` coins`)]
                    }).catch(() => {
                        userNotDmAble = true
                    })
                } else {
                    ...
                    if (result.planetXp + Math.round(Math.random() * (5 - 1) + 1) >= result.requiredPlanetXp) {
                        ...
                        if (result.dmNotifs === true) {
                            await interaction.user.send({
                                embeds: [new EmbedBuilder().setColor(16777215).setTitle('Congrats, You leveled up!').setDescription(`You are now **planet level ${result.planetLevel}**`)]
                            }).catch(() => {
                                userNotDmAble = true
                            })
                        } else {

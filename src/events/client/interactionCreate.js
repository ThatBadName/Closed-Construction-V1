    ...
    async execute(interaction, client) {
        if (interaction.isChatInputCommand()) {
            ...
            try {
                ...
                console.error(error)
                const webhookClient = new WebhookClient({ url: `https://discord.com/api/webhooks/1023674577374691419/Ot2AQXk_04n37gOsZNrFN935Ai3rGfCqKv5qRHHmMkT32IAsl0OPjtOSI8oOSBVuFNI0` })
                webhookClient.send({
                    embeds: [
                        new EmbedBuilder()
            ...
            if (result.xp + Math.round((Math.random() * (10 - 1) + 1 / 100 * result.xpBoost) + Math.random() * (10 - 1) + 1) >= result.requiredXp) {
                ...
                if (result.dmNotifs === true) {
                    interaction.user.send({
                        embeds: [new EmbedBuilder().setColor(colours.blend).setTitle('Congrats, You leveled up!').setDescription(`You are now **level ${result.level}**. As a reward you have been given \`${reward}\` coins`)]
                    }).catch(() => {
                        userNotDmAble = true
                    })
                } else {
                    ...
                    if (result.planetXp + Math.round(Math.random() * (5 - 1) + 1) >= result.requiredPlanetXp) {
                        ...
                        if (result.dmNotifs === true) {
                            await interaction.user.send({
                                embeds: [new EmbedBuilder().setColor(colours.blend).setTitle('Congrats, You leveled up!').setDescription(`You are now **planet level ${result.planetLevel}**`)]
                            }).catch(() => {
                                userNotDmAble = true
                            })
                        } else {
            ...
            } else if (interaction.customId.includes('marketOffer-')) {
                const functions = require('../../commandFunctions')
                const blks = await functions.blacklistCheck(interaction.user.id, interaction.guild.id, interaction)
                if (blks === true) return
                const main = await functions.checkMaintinance(interaction)
                if (main === true) return
                const cldn = await functions.cooldownCheck(interaction.user.id, 'accept', 3, interaction)
                if (cldn === true) return
                functions.createRecentCommand(interaction.user.id, 'accept', `${interaction.customId}`, interaction)

                const command = require(`../../things/marketOfferCode`)
                command.offerRegular(interaction)
            } else if (interaction.customId.includes('offer-')) {
                const functions = require('../../commandFunctions')
                const blks = await functions.blacklistCheck(interaction.user.id, interaction.guild.id, interaction)
                if (blks === true) return
                const main = await functions.checkMaintinance(interaction)
                if (main === true) return
                const cldn = await functions.cooldownCheck(interaction.user.id, 'accept', 2, interaction)
                if (cldn === true) return
                functions.createRecentCommand(interaction.user.id, 'accept', `${interaction.customId}`, interaction)

                const command = require(`../../things/marketOfferCode`)
                command.offerDark(interaction)
            ...
            if (interaction.customId.includes('offer')) return

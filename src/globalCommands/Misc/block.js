...
module.exports = {
    ...
    async execute(
        ...
        if (interaction.options.getSubcommand() === 'list') {
            ...
            let firstEmbed
            if (blocklistEmbeds.length === 0) {
                return interaction.reply({
                    embeds: [new EmbedBuilder().setColor('0x' + colours.blend).setTitle('You have not blocked anyone, yet....')],
                    fetchReply: true
                })
            } else {
                firstEmbed = await interaction.reply({

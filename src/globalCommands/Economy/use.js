    ...
    async execute(interaction) {
        ...
        if (interaction.options.getUser('user')) {
            ...
            if (user.id === interaction.user.id) return interaction.editReply({
                embeds: [
                    new EmbedBuilder()
                    .setTitle('You cannot use this on yourself')
                    .setColor(16777215)
                ]
            })
        ...
        if (itemFound.id === 'spellbound berries') amount = 1
        ...
        if (itemFound.id === 'cheque') {
        ...
        } else if (itemFound.id === 'charged xp coin' || itemFound.id === 'dark charged xp coin') {
            const amountToAdd = Math.floor(Math.random() * (100 - 50) + 50) * amount
            userProfile.xpBoost += amountToAdd
            userProfile.save()

            const date = new Date()
            date.setMinutes(date.getMinutes() + (30 * amount))
            xpBoostSchema.create({
                userId: interaction.user.id,
                expires: date,
                increase: amountToAdd
        ...
        } else if (itemFound.id === 'splinter coin') {
            let itemList = require('../../things/items/allItems')
            let filteredItems = itemList.filter(i => i.type === 'Collectable' && i.id !== 'bad coin' && i.id !== 'ihate coin')
            let itemGiving = filteredItems[Math.floor(Math.random() * filteredItems.length)]

            const inventory = await inventorySchema.findOne({
                userId: interaction.user.id,
                itemId: itemGiving.id
            })
            if (!inventory) await inventorySchema.create({
                userId: interaction.user.id,
                itemId: itemGiving.id,
                amount: amount,
                item: itemGiving.name,
                emoji: itemGiving.emoji
            })
            else {
            ...
            interaction.editReply({
                embeds: [
                    new EmbedBuilder()
                    .setTitle(`You used a Splinter Coin`)
                    .setDescription(`You used ${amount.toLocaleString()} Splinter Coin${amount > 1 ? 's' : ''} and were given ${amount.toLocaleString()}x ${itemGiving.emoji}${itemGiving.name}`)
                    .setColor(16777215)
                ]
            })
                        ...
                        if (result.dmNotifs === true) {
                            try {
                                interaction.user.send({
                                    embeds: [new EmbedBuilder().setColor(16777215).setTitle('Congrats, You leveled up!').setDescription(`You are now **level ${userProfile.level.toLocaleString()}**. As a reward you have been given \`${reward.toLocaleString()}\` coins`)]
                                })
                            } catch (err) {
        ...
        } else if (itemFound.id === 'spellbound berries') {
            const randomGood = Math.round(Math.random() * (4 - 1) + 1)
            if (randomGood === 1) {
                const amountAdding = Math.round(Math.random() * (15 - 10) + 10)
                userProfile.xpBoost += amountAdding
                userProfile.save()
                ...
                interaction.editReply({
                    embeds: [
                        new EmbedBuilder()
                        .setTitle('You ate some spellbound berries and...')
                        .setDescription(`You got an XP Booster of \`${amountAdding.toLocaleString()}%\`. Your total XP boost is now \`${userProfile.xpBoost.toLocaleString()}%\``)
                        .setColor(16777215)
                    ]
                })
